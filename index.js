...
document.querySelector('img').remove();
 ...
 while (document.querySelector('p') !== null)
document.querySelector('p').remove();

  
//Another way to clear the content is using the .innerHTML method
...
function showMeme() {
  ...
  //Creating the element 'img' to be added
  let image = document.createElement('img');
  
  //Assigning the image source to the ramdom meme URL
  image.src = randomMemeUrl;
  
  //Selecting the first h3 heading
  let firstH3 = document.querySelector('h3:first-of-type');
  
  //Adding the image between the first and second h3 headings
  firstH3.parentNode.insertBefore(image, firstH3.nextSibling);
...
function showJoke() {
  ...
  //Finding the location at which we want to insert the joke in the HTML documenty
  const secondH3 = document.querySelector('h3:nth-of-type(2)');
  secondH3.parentNode.insertBefore(joke, secondH3.nextSibling);
...
function showQuote() {
  ...
  // Assigning bothe QUOTE and AUTHOR to variables
  let quote = randomQuote.quote;
  let author = randomQuote.author;
  
  // Creating a string with the final quote formatting
  let finalQuote = quote.concat(" - ", author);
  
  //Now we're gonna insert the quote in its respectif section in the HTML document
  let pQuote = document.createElement('p');
  pQuote.textContent = finalQuote;
  
document.querySelector('.quote').appendChild(pQuote);
...
document.querySelector('.riddle').appendChild(questionElement);
  document.querySelector('.riddle').appendChild(answerElement);
}
...
function revealAnswers() {
  ...
  let answerEmplacement = document.querySelector('#answerId');
  
if (answerEmplacement === null)
  alert('Please press the Riddle Me button first!');
else if(answerEmplacement.hidden === true){ 
  answerEmplacement.hidden = false;
}else if(answerEmplacement.hidden === false){
...
// Just a little helper function that returns a random number between 0 and the array length (meme, quotes, jokes and riddles)
function rn(len) {
